...
namespace X13 {
  ...
  public partial class App : Application {
      ...
      lock(_icons) {
        if(!_icons.TryGetValue(icData, out rez)) {
          if(icData.StartsWith("data:image/png;base64,")) {
            ...
            var url = new Uri("pack://application:,,,/Desk;" + icData, UriKind.Absolute);
    ...
    private void LoadIcon(string name, string path) {
      var decoder = new PngBitmapDecoder(new Uri("pack://application:,,,/Desk;component/Images/" + path, UriKind.Absolute), BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.None);
